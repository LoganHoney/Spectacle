"""Rebuilds js/vendor/forms/insp4pt-fillable.pdf from tools/masters/4p-fillable.pdf
(the real Acrobat-prepared fillable 4-Point form the user supplied) by running
tools/build_4point_fields.js in a real browser via pdf-lib — see that file's
header comment for what it patches and why.
...
machine (documented elsewhere in this project), and sidestepping the network
entirely for these two large local files is more reliable than retrying.
"""
import base64
import json
import sys
...
MASTER_PATH = ROOT + r"\tools\masters\4p-fillable.pdf"
BUILD_SCRIPT_PATH = ROOT + r"\tools\build_4point_fields.js"
PDF_LIB_PATH = ROOT + r"\js\vendor\pdf-lib.min.js"
OUT_PATH = ROOT + r"\js\vendor\forms\insp4pt-fillable.pdf"
...
def main():
    ...
    try:
        ...
        for label, path in [("pdf-lib", PDF_LIB_PATH), ("build_4point_fields.js", BUILD_SCRIPT_PATH)]:
        ...
        build_expr = f"""
            (async () => {{
              ...
              const outBytes = await window.build4PointFields(bytes);
              let binary = '';
